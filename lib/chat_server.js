...
function handleJoining (socket) {
  socket.on('join', (room, nickname) => {
    ...
    // check nickname
    if (isOccupiedNickname(nickname)) {
      ...
      const usersInRoom = queryUsersInRoom(room)
      socket.emit('system', `Online: ${usersInRoom.join(', ')}`)
...
/* find others in current room */
function queryUsersInRoom (room) {
  const others = []
  for (const user of Object.values(users)) {
    if (user.room === room) {
      others.push(user.nickname)
    }
  }
...
function isOccupiedNickname (nickname) {
